...
export default function ClerkDashboardSessionsPage() {
  const { currentUser, getAllCases } = useStore()
  const allCases = getAllCases()

  const myCases = allCases.filter((c) => c?.courtRegistry?.assignedClerkId === currentUser?.id)
  ...
  const now = new Date()
  const upcomingSessions = sessions.filter((session) => session.sessionDate.getTime() >= now.setHours(0, 0, 0, 0))
  const completedSessions = sessions.filter((session) => session.sessionDate.getTime() < now.getTime())

  return (
    <DashboardLayout allowedRoles={["clerk"]} title="Session List">
      <div className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-sm text-muted-foreground">Recorded Cases</div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Upcoming Clerk Sessions</CardTitle>
            <CardDescription>Hearing dates attached to your assigned cases.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {upcomingSessions.length > 0 ? (
              ...
              <div className="rounded-lg border p-6 text-center text-muted-foreground">
                No session dates are attached to your assigned clerk cases yet.
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Already-recorded hearing dates on your assigned cases.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {completedSessions.length > 0 ? (
              ...
              <div className="rounded-lg border p-6 text-center text-muted-foreground">
                No past sessions are recorded for your assigned cases yet.
              </div>
            )}
          </CardContent>
        </Card>
